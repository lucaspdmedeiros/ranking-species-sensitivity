# Solves the stochastic food chain model in Upadhyay 2000 
...
food_chain_stochastic <- function(N, p, s, times) {
  # set up diffeqr
  de <- diffeqr::diffeq_setup()
  # define deterministic part
  f <- function(u, p, t) {
    du1 <- p[1] * u[1] * (1 - (u[1] / p[2])) - u[1] * ((p[3] * u[2]) / (1 + p[4] * u[1]))
    du2 <- -p[5] * u[2] + p[6] * u[1] * u[2] - u[2] * ((p[7] * u[3]) / (1 + p[8] * u[2]))
    du3 <- -p[9] * u[3] + p[10] * u[2] * u[3]
