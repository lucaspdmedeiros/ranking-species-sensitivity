# Code for Fig S19: comparison between expected sensitivity, 
# eigenvector, rate of change, and abundance approaches to rank
# species forecast errors for multiple models
...
# compute correlation between forecast errors and different rankings ------------------------------
...
for (i in 1:length(func_name)) {
  # load results
  if (normalize) {
    load(file = paste("results/forecast_analyses/forecasts_sensitivities_", func_name[i], "_", n_sp[i], "_sp_",
  ...
  # data frames with forecast errors and prediction of each ranking approach
  df_expect_sens <- cbind(df[ , paste("epsilon_", 1:n_sp[i], sep = "")], 
                          df[ , paste("expected_sensitivity_", 1:n_sp[i], sep = "")])
  df_eigen_align <- cbind(df[ , paste("epsilon_", 1:n_sp[i], sep = "")], 
                          df[ , paste("eigen_alignment_", 1:n_sp[i], sep = "")])
  df_rate_change <- cbind(df[ , paste("epsilon_", 1:n_sp[i], sep = "")], 
                          df[ , paste("delta_x", 1:n_sp[i], sep = "")])
  df_abundance <- cbind(df[ , paste("epsilon_", 1:n_sp[i], sep = "")], 
...
if (save_plots) {
  if (normalize) {
    ggsave(paste("figs/fig_correlation_rankings_forecast_errors_", jacobian, "_normalized_", pert_dist, "_pert_", pert_magnitude, "_pert_magn_", 
                 k_type, "_time_step_k_", k, "_", noise, "_noise", ".pdf", sep = ""), 
           fig, width = 38, height = 16, units = "cm")
  } else {
    ggsave(paste("figs/fig_correlation_rankings_forecast_errors_", jacobian, "_", pert_dist, "_pert_", pert_magnitude, "_pert_magn_", 
                 k_type, "_time_step_k_", k, "_", noise, "_noise", ".pdf", sep = ""), 
           fig, width = 38, height = 16, units = "cm")
  }
}
